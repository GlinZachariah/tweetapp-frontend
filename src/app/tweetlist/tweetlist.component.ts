...
export class TweetlistComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.service.getAllTweets().subscribe((res: IncomingResponse) => {
      if (res.code == 200) {
        this.tweetsList = res.message as TweetEntity[];
        let userName = this.route.snapshot.firstChild?.params?.id != null ? this.route.snapshot.firstChild.params.id : "";
        if (userName != "") {
          this.tweetsList = this.tweetsList.filter((tweet) => tweet.userId == userName);
        }
      }
    });
  }
