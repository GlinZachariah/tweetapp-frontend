...
export class TweetappComponent implements OnInit {
  ...
  likeTweet(like: boolean) {
    ...
    this.service.likeTweet(this.currentUser, this.tweet.tweetId).subscribe((res: IncomingResponse) => {
      if (res.code == 200) {
        if (like) {
          this.tweet.userIdLiked.push(this.currentUser);
          this.tweet.likeCounter += 1;
        } else {
          this.tweet.userIdLiked.splice(this.tweet.userIdLiked.indexOf(this.currentUser), 1);
          this.tweet.likeCounter -= 1;
        }
      }
    })
  ...
  deleteTweet() {
    this.service.deleteTweet(this.currentUser, this.tweet.tweetId).subscribe((res: IncomingResponse) => {
      if (res.code == 200) {
